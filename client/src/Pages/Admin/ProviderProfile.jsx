  ...
  return (
    <div>
      <div className={data.coverPhoto ? `bg-black w-full h-[300px] lg:h-[500px] flex relative justify-center` : 'bg-slate-300 w-full h-[300px] lg:h-[500px] flex relative justify-center'} >
        ...
        <Link to="/vendor/editProfile" className='bg-white border-black border-2 absolute right-16 top-14 w-48 h-16 rounded-2xl flex items-center justify-center text-xl font-semibold hover:shadow-xl uppercase hover:bg-[#E1EDF8] hover:scale-105'>edit profile</Link>
            ...
            <div className='grid grid-flow-col gap-3 overflow-x-scroll show-scrollbar mb-10 col-span-3'>
              {category.map((service) => (
                <div key={service.id}>
                  ...
                  <button className='bg-red-500 text-white rounded' onClick={() => handleDelete(service)}>Delete</button>
                </div>
              ))}
            </div>
          ) : (
