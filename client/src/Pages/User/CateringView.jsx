...
function CateringView() {
  ...
  const getCatering = async () => {
    try {
      const response = await axios.get('/cateringedisplay', {
        headers: {
  ...
  const datas = catering.map((data) => {
    return (
      !catView && (
        <div key={data._id} className="min-h-screen bg-gray-100 flex justify-center items-center mt-8">
              ...
              <div className="container mx-auto p-9 bg-gray-300 max-w-sm rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition duration-300 mt-0">
                <div className="flex justify-between items-center mt-0">
                  <table className="text-xs my-3">
                    <tbody>
                      <tr>
                        <td className="px-2 py-2 text-black text-lg font-extrabold">Type</td>
                      ...
                      <tr>
                        ...
                        <td className="px-2 py-2 text-lg font-bold">{data.rent}</td>
                      </tr>
                      <tr>
                        <td>
                          <button
  ...
  return (
    <div>
      ...
      {catView && <Photosingle data={{ ...catering, catview: true }} />}
    </div>
  );
}
